...
boardRoutes.get("/:projectId/board", async (c) => {
  ...
  const statuses = await db
    .select({ id: workflowStatuses.id, name: workflowStatuses.name, color: workflowStatuses.color, category: workflowStatuses.category, sortOrder: workflowStatuses.sortOrder })
  ...
  const taskRows = await db
    .select({
      id: tasks.id, title: tasks.title, workflowStatusId: tasks.workflowStatusId, startDate: tasks.startDate, dueDate: tasks.dueDate,
      estimatedHours: tasks.estimatedHours, budgetCost: tasks.budgetCost, featureId: tasks.featureId,
      assigneeId: users.id, assigneeName: users.name, assigneeImage: users.image,
    })
    .from(tasks)
  ...
  const customFieldRows = taskIds.length
    ? await db
        .select({ entityId: customFieldValues.entityId, fieldId: customFields.id, fieldName: customFields.fieldName, fieldType: customFields.fieldType, value: customFieldValues.value })
  ...
  const columns = statuses.map((status) => ({
    id: status.id, name: status.name, color: status.color, category: status.category, sortOrder: status.sortOrder,
    tasks: taskRows
      .filter((t) => t.workflowStatusId === status.id)
      .map((t) => ({
        id: t.id, title: t.title, featureId: t.featureId,
        ...
        actualHours: actualHoursMap.get(t.id) ?? 0, budgetCost: t.budgetCost,
        customFields: taskCustomFieldsMap.get(t.id) ?? [],
      })),
  }));

  return c.json({
    project: { id: project.id, name: project.name, status: project.status, theme: project.themeId ? { id: project.themeId, name: project.themeName } : null, autoDates, progress },
    columns,
  });
});
