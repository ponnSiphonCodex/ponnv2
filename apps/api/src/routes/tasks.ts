...
taskRoutes.patch("/:id/status", async (c) => {
  ...
  const [target] = await db.select().from(workflowStatuses).where(eq(workflowStatuses.id, workflowStatusId));
  if (!target) return c.json({ error: "ไม่พบ Status ปลายทาง" }, 400);
  const [updated] = await db.update(tasks).set({ workflowStatusId, updatedBy: user.id }).where(eq(tasks.id, taskId)).returning();
  await db.insert(activityLogs).values({ entityType: "task", entityId: taskId, userId: user.id, action: "status_changed", fieldChanged: "workflow_status_id", oldValue: String(existing.workflowStatusId), newValue: String(workflowStatusId) });
...
taskRoutes.post("/:id/worklogs", async (c) => {
  ...
  const db = createDb(c.env.DB);
  const user = c.get("user");
  const [log] = await db.insert(taskWorklogs).values({ taskId, userId: user.id, workDate: new Date(body.workDate), hoursSpent: body.hoursSpent, note: body.note, createdBy: user.id, updatedBy: user.id }).returning();
