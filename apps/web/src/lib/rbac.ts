...
export async function getRolesForUser(db: DbClient, userId: string): Promise<UserRole[]> { const r=await db.prepare(`SELECT sr.id,sr.role_name AS "roleName",sr.module FROM user_roles ur JOIN system_roles sr ON ur.role_id=sr.id WHERE ur.user_id=?`).bind(userId).all<UserRole>(); return r.results??[]; }
