 ...
 *
 * ระบบ session แบบเขียนเอง (แทน NextAuth ที่ทำ error=Configuration บน Cloudflare)
 * ใช้ JavaScript ธรรมดา + Web Crypto (HMAC-SHA256) — ไม่พึ่ง lib auth ภายนอกเลย
 *
 * Token format: base64url(JSON payload) + "." + base64url(HMAC signature)
 * เก็บใน cookie ชื่อ "session" (HttpOnly, Secure, SameSite=Lax)
 */
...
export const SESSION_MAX_AGE = 30 * 24 * 60 * 60; // 30 วัน
...
/** สร้าง session token (เซ็นด้วย HMAC) */
export async function createSessionToken(
  secret: string,
  data: { sub: string; email: string; name: string | null },
...
/** สร้างค่า Set-Cookie header สำหรับ set session */
export function buildSessionCookie(token: string): string {
  return `${SESSION_COOKIE}=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${SESSION_MAX_AGE}`;
}

/** สร้างค่า Set-Cookie header สำหรับลบ session (logout) */
