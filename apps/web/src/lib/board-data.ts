...
export type BoardTask = { id: number; title: string; workflowStatusId: number | null; sortOrder: number; assignee: { id: string; name: string | null } | null; priority: { name: string; color: string | null } | null; estimatedHours: number | null; actualHours: number; dueDate: number | null; projectName?: string | null; projectId?: number | null; productName?: string | null; productId?: number | null; featureName?: string | null; featureId?: number | null };
...
export async function getAllProjectsBoard(db: DbClient, ids: number[] | null): Promise<BoardData> {
  ...
  const tk = await db.prepare(
    `SELECT t.id, t.title, t.workflow_status_id, t.estimated_hours, t.due_date, t.sort_order, t.assignee_id,
       ...
       ws.category AS category, pr.name AS project_name, pr.id AS project_id, pd.name AS product_name, pd.id AS product_id, f.name AS feature_name, f.id AS feature_id,
       COALESCE(wl.actual_hours, 0) AS actual_hours
     FROM tasks t
     LEFT JOIN users u ON t.assignee_id=u.id
     LEFT JOIN priorities p ON t.priority_id=p.id
     ...
     LEFT JOIN products pd ON pr.product_id=pd.id
  ...
  const columns: BoardColumn[] = CANON.map((c, i) => ({ id: -(i + 1), name: c.name, color: c.color, category: c.key,
    tasks: rows.filter((t) => (t.category ?? "backlog") === c.key).map((t) => {
      ...
      return { id: t.id, title: t.title, workflowStatusId: t.workflow_status_id, sortOrder: t.sort_order,
        ...
        projectName: t.project_name ?? null, projectId: t.project_id ?? null, productName: t.product_name ?? null, productId: t.product_id ?? null, featureName: t.feature_name ?? null, featureId: t.feature_id ?? null } as any;
    }) }));
  const total = rows.length; const denom = total - drop;
