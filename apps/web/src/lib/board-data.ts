 ...
 * Query ข้อมูล Kanban board จาก D1 ตรง ๆ (server component) ไม่พึ่ง API worker แยก
 */
import { and, eq, inArray, sql } from "drizzle-orm";
import { customFields, customFieldValues, features, projects, tasks, users, workflowStatuses, taskWorklogs } from "@/db";
...
export type BoardData = {
  project: { id: number; name: string; status: string | null; progress: { total: number; done: number; percent: number } };
  columns: BoardColumn[];
};
...
export async function getBoardData(db: DbClient, projectId: number): Promise<BoardData | null> {
  ...
  const taskRows = await db
    .select({
      id: tasks.id, title: tasks.title, workflowStatusId: tasks.workflowStatusId, estimatedHours: tasks.estimatedHours,
  ...
  if (taskIds.length) {
    const hourRows = await db
      .select({ taskId: taskWorklogs.taskId, actualHours: sql<number>`coalesce(sum(${taskWorklogs.hoursSpent}), 0)`.mapWith(Number) })
      .from(taskWorklogs)
...
// อ่านสิทธิ์ module ของ user (ใช้ในหน้า portal)
