...
export const systemRoles = sqliteTable("system_roles", {
  id: integer("id").primaryKey({ autoIncrement: true }), roleName: text("role_name").notNull(),
...
export const workflowStatuses = sqliteTable("workflow_statuses", { id: integer("id").primaryKey({ autoIncrement: true }), projectId: integer("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), name: text("name").notNull(), color: text("color"), category: text("category", { enum: ["todo", "doing", "done"] }).notNull(), sortOrder: integer("sort_order").notNull().default(0), ...auditFields() }, (t) => ({ projectIdx: index("workflow_statuses_project_idx").on(t.projectId) }));
...
export const customFieldValues = sqliteTable("custom_field_values", { id: integer("id").primaryKey({ autoIncrement: true }), customFieldId: integer("custom_field_id").notNull().references(() => customFields.id, { onDelete: "cascade" }), entityId: integer("entity_id").notNull(), value: text("value"), ...auditFields() }, (t) => ({ uniqFieldEntity: uniqueIndex("custom_field_values_field_entity_uniq").on(t.customFieldId, t.entityId), entityIdx: index("custom_field_values_entity_idx").on(t.entityId) }));
