...
export function BoardClient({ board, projects, users, priorities, features, tags, canWrite, aggregate = false, isAdmin = false }: {
  ...
  const ALL_CARD_FIELDS=[["project","Project"],["product","Product"],["feature","Feature"],["status","Status"],["assignee","ผู้รับผิดชอบ"],["priority","Priority"],["start","Start Date"],["due","Due Date"],["hours","Actual / Estimate Hours"]] as const;
  ...
  const saveFields=(n:string[])=>{setCardFields(n);localStorage.setItem("kanban-card-fields",JSON.stringify(n))}; const toggleField=(f:string)=>saveFields(cardFields.includes(f)?cardFields.filter(x=>x!==f):[...cardFields,f]); const moveField=(i:number,d:number)=>{const n=[...cardFields],j=i+d;if(j<0||j>=n.length)return;[n[i],n[j]]=[n[j],n[i]];saveFields(n)};
  ...
  return (
    <div style={{ padding: 20 }}>
            ...
            <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 9, overflowY: "auto", minHeight: 60 }}>
              ...
              {col.tasks.filter(matches).map((t) => (
                <div key={t.id} draggable={canWrite} onDragStart={() => setDragId(t.id)} onDragEnd={() => { setDragId(null); setOverCol(null); }} onClick={() => t.id > 0 && setDrawerTask(t.id)}
                  ...
                  <div style={{fontSize:10.5,color:"#9AA0A6",marginBottom:3}}>TASK-{String(t.id).padStart(5,"0")}</div><div style={{ fontSize: 13.5, fontWeight: 600, color: "#1F2937", marginBottom: 6 }}>{t.title}</div>{cardFields.map(f=>f==="product"&&t.productName?<CardLine key={f} label="Product" value={t.productName}/>:f==="feature"&&t.featureName?<CardLine key={f} label="Feature" value={t.featureName}/>:f==="status"?<CardLine key={f} label="Status" value={col.name}/>:f==="start"&&t.startDate?<CardLine key={f} label="Start" value={new Date(t.startDate*1000).toISOString().slice(0,10)}/>:f==="due"&&t.dueDate?<CardLine key={f} label="Due" value={new Date(t.dueDate*1000).toISOString().slice(0,10)}/>:null)}
      ...
      {showCardSettings&&<div style={{position:"fixed",inset:0,zIndex:95,background:"rgba(0,0,0,.35)",display:"grid",placeItems:"center"}}><div className="card" style={{padding:22,width:"min(420px,92vw)"}}><h3 style={{marginTop:0}}>ตั้งค่าข้อมูลบน Kanban Card</h3>{ALL_CARD_FIELDS.map(([k,l])=>{const i=cardFields.indexOf(k);return <div key={k} style={{display:"grid",gridTemplateColumns:"24px 1fr auto",gap:8,alignItems:"center",padding:"7px 0"}}><input type="checkbox" checked={i>=0} onChange={()=>toggleField(k)}/><span>{l}</span>{i>=0&&<span style={{display:"flex",gap:4}}><button className="btn-ghost" style={{padding:"3px 7px"}} onClick={()=>moveField(i,-1)}>↑</button><button className="btn-ghost" style={{padding:"3px 7px"}} onClick={()=>moveField(i,1)}>↓</button></span>}</div>})}<div style={{display:"flex",justifyContent:"flex-end",marginTop:14}}><button className="btn-pink" onClick={()=>setShowCardSettings(false)}>เสร็จ</button></div></div></div>}
...
function CardLine({label,value}:{label:string;value:string}){return <div style={{fontSize:12,color:"#6B7280",marginBottom:3}}><b style={{color:"#374151"}}>{label}:</b> {value}</div>}
