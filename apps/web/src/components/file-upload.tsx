 ...
 * คอมโพเนนต์อัปโหลดไฟล์ไป Google Drive ผ่าน Apps Script (client component)
 * ใช้ <input type="file" /> → แปลง Base64 → POST ไป Apps Script (ดู lib/upload.ts)
 *
 * วิธีใช้:
      ...
      {result?.ok && (
        <p style={{ fontSize: 13, color: "#059669", marginTop: 12 }}>
          ...
          {result.url && (
            <>
              {" · "}
              <a href={result.url} target="_blank" rel="noopener noreferrer" style={{ color: "#001D58" }}>เปิดไฟล์</a>
            </>
          )}
          {result.fileId && <span style={{ color: "#9AA0A6" }}> (id: {result.fileId})</span>}
        </p>
      )}

      {result && !result.ok && (
        <p style={{ fontSize: 13, color: "#B91C1C", marginTop: 12 }}>❌ อัปโหลดไม่สำเร็จ: {result.error}</p>
      )}
    </div>
  );
}
