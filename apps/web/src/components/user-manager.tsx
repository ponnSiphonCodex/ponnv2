...
export function UserManager() {
  ...
  const reqTotal = guestUsers.length;
  useEffect(() => {
    if (loading) return;
    localStorage.setItem("pending_req", String(reqTotal));
    window.dispatchEvent(new CustomEvent("pending-req-changed", { detail: reqTotal }));
  }, [reqTotal, loading]);
