...
export function ProjectWizard({products,features,users,initial={},projectId}:Props){
 ...
 return <div style={{padding:24,maxWidth:1120}}><div style={{display:"grid",gap:18}}>
  ...
  <section className="card" style={{padding:24}}><h3 className="form-section-title">2. Timeline</h3><p style={{color:"#6B7280",margin:0}}>Timeline ของ Project คำนวณจากช่วงวัน (วันเริ่ม–กำหนดส่ง) ของ Task ในโครงการโดยอัตโนมัติ · เพิ่ม/แก้วันของ Task ได้ที่หน้า Kanban หรือ Gantt Chart</p></section>
  ...
  <section className="card" style={{padding:24}}><h3 className="form-section-title">4. เอกสาร Project</h3><FileUpload onUploaded={x=>x.ok&&setFiles(v=>[...v,{fileId:x.fileId,url:x.url}])}/>{files.map((f:any,i:number)=><div key={i} style={{marginTop:8}}><a href={f.url} target="_blank">เปิดไฟล์ {i+1}</a></div>)}</section>
