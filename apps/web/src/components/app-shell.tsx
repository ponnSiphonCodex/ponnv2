 ...
 * Layout หลัก — Sidebar ซ้าย (Hamburger collapse/expand), จำสถานะใน localStorage
 * เมนู admin-only ซ่อนถ้าไม่ใช่ admin
 */
...
export function AppShell({ children, active, user, isAdmin, roleLabel }: { children: ReactNode; active: string; user: { name: string | null; email: string }; isAdmin: boolean; roleLabel: string }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100dvh", background: "#F4F4F6" }}>
      <aside style={{ width, minWidth: width, background: NAVY, color: "#fff", display: "flex", flexDirection: "column", transition: ready ? "width 0.18s ease, min-width 0.18s ease" : "none", position: "sticky", top: 0, height: "100dvh" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 14px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          <button onClick={toggle} aria-label="toggle menu" style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", padding: 6, borderRadius: 8, display: "flex", flexShrink: 0 }}><Icon d="M3 12h18M3 6h18M3 18h18" /></button>
        ...
        <nav style={{ flex: 1, padding: "12px 8px", display: "flex", flexDirection: "column", gap: 4, overflowY: "auto" }}>
          {visibleMenus.map((m) => {
            ...
            return (
              <a key={m.key} href={m.href} title={collapsed ? m.label : undefined} style={{ display: "flex", alignItems: "center", gap: 12, padding: collapsed ? "11px 0" : "11px 12px", justifyContent: collapsed ? "center" : "flex-start", borderRadius: 10, textDecoration: "none", color: a ? "#fff" : "rgba(255,255,255,0.7)", background: a ? PINK : "transparent", fontSize: 14, fontWeight: a ? 600 : 500, whiteSpace: "nowrap" }}>
                {m.icon}{!collapsed && <span>{m.label}</span>}
              </a>
            );
          })}
        </nav>
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", padding: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: collapsed ? 0 : 10 }}>
            ...
            {!collapsed && (<div style={{ overflow: "hidden" }}><div style={{ fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name || user.email}</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", whiteSpace: "nowrap" }}>{roleLabel}</div></div>)}
          </div>
          <a href="/api/logout" title="ออกจากระบบ" style={{ display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "flex-start", gap: 10, padding: collapsed ? "9px 0" : "9px 10px", borderRadius: 8, color: "rgba(255,255,255,0.75)", textDecoration: "none", fontSize: 13 }}>
            <Icon d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9" />{!collapsed && <span>ออกจากระบบ</span>}
          </a>
        </div>
      </aside>
...
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  return (
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "24px 28px", borderBottom: "1px solid #E5E7EB", background: "#fff", gap: 16, flexWrap: "wrap" }}>
      <div><h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: NAVY }}>{title}</h1>{subtitle && <p style={{ margin: "4px 0 0", fontSize: 14, color: "#6B7280" }}>{subtitle}</p>}</div>
      {actions}
    </header>
  );
}
