"use client";
/**
 * apps/web/src/components/team-manager.tsx
 * จัดการผู้ใช้ + สิทธิ์ (admin) — โหลด /api/users, กด toggle role ผ่าน /api/user-roles
 */
...
export function TeamManager() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) { setError("โหลดข้อมูลไม่สำเร็จ (ต้องเป็น admin)"); setLoading(false); return; }
      const data = (await res.json()) as { users: User[]; allRoles: AllRole[] };
      setUsers(data.users);
  ...
  async function toggleRole(userId: string, roleId: number, has: boolean) {
      ...
      {users.map((u) => (
        <div key={u.id} style={{ background: "#fff", borderRadius: 12, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <div>
              <strong style={{ color: "#001D58" }}>{u.name || u.email}</strong>
              <div style={{ fontSize: 12, color: "#9AA0A6" }}>{u.email}</div>
            </div>
            <div style={{ fontSize: 12, color: "#6B7280" }}>
              {u.roles.length ? u.roles.map((r) => r.roleName).join(", ") : "ยังไม่กำหนดสิทธิ์"}
            </div>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {allRoles.map((r) => {
              const has = u.roles.some((ur) => ur.roleId === r.id);
              const isBusy = busy === `${u.id}:${r.id}`;
              return (
                <button
                  key={r.id}
                  onClick={() => toggleRole(u.id, r.id, has)}
                  disabled={isBusy}
                  style={{
                    padding: "6px 14px",
                    borderRadius: 20,
                    border: has ? "none" : "1px solid #E5E7EB",
                    background: has ? "#001D58" : "#fff",
                    color: has ? "#fff" : "#6B7280",
                    fontSize: 13,
                    cursor: "pointer",
                ...
                >
                  {has ? "✓ " : "+ "}{r.roleName} <span style={{ opacity: 0.6, fontSize: 11 }}>({r.module})</span>
                </button>
              );
            })}
          </div>
