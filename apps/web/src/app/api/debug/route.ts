...
export async function GET() {
  ...
  if (envCheck.DB_BOUND) {
    try { const db = createDb(env.DB); const rows = await db.select().from(users).limit(5); dbCheck = { ok: true, userCount: rows.length }; }
    catch (err) { dbCheck = { ok: false, error: err instanceof Error ? err.message : String(err) }; }
  } else { dbCheck = { ok: false, error: "env.DB ไม่มีค่า" }; }
