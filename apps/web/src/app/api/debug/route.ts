...
export async function GET() {
  ...
  if (envCheck.DB_BOUND) { try { const db = createDb(env.DB); const rows = await db.select().from(users).limit(50); dbCheck = { ok: true, userCount: rows.length }; } catch (e) { dbCheck = { ok: false, error: e instanceof Error ? e.message : String(e) }; } } else { dbCheck = { ok: false, error: "env.DB ไม่มีค่า" }; }
