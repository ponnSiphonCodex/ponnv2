...
export async function GET(req: NextRequest) {
  ...
  const r = await createDatabase(env).prepare(
  ...
  for (const t of rows) {
    ...
    await notify({ d1: createDatabase(env), env, targetUserId: t.assignee_id, actorId: null, actionType: "Due_Soon", referenceType: "task", referenceId: t.id, message: `⏰ งาน "${t.title}" ครบกำหนดในอีก ~${hrs} ชม.` });
    sent++;
  }
