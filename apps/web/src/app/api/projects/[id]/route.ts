...
export async function PATCH(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{await c.d1.prepare(`DELETE FROM project_category_tags WHERE project_id=?`).bind(pid).run();for(const cat of b.categories??[])await c.d1.prepare(`INSERT INTO project_category_tags(project_id,category) VALUES(?,?)`).bind(pid,cat).run()}catch{}
