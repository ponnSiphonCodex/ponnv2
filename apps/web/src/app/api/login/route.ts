 ...
 * POST { email, password } → verify กับ D1 → set session cookie → { ok: true }
 * (แทน NextAuth Credentials — เขียนเอง JS ธรรมดา)
 */
...
export async function POST(req: NextRequest) {
  ...
  if (!email || !password) {
    return Response.json({ ok: false, error: "อีเมลและรหัสผ่านห้ามว่าง" }, { status: 400 });
  }
