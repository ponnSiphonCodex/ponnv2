...
export async function POST(req: NextRequest) {
  ...
  if (imp) {
    const target = await env.DB.prepare(`SELECT email, name FROM users WHERE id=?`).bind(imp).first<any>();
    await logActivity(env.DB, { referenceType: "impersonation", referenceId: 0, userId: sess.sub, action: "Impersonate_Start", newValue: `${sess.email} → ${target?.email ?? imp}` });
    await notifyAdminChat(env, `⚠️ <b>Impersonation</b>\n${sess.name ?? sess.email} เริ่มสวมบทบาทเป็น ${target?.name ?? target?.email ?? imp}`);
  } else {
    await logActivity(env.DB, { referenceType: "impersonation", referenceId: 0, userId: sess.sub, action: "Impersonate_End", newValue: sess.email });
  }
