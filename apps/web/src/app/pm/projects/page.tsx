...
export default async function ProjectsPage({searchParams}:{searchParams:Promise<{view?:string}>}){
 ...
 const ids=await visibleProjectIds(a.d1,a.scope); let where=view==="active"?`LOWER(COALESCE(p.status,'active')) NOT IN ${inactive}`:`1=1`; const binds:any[]=[];
 if(ids){if(!ids.length)where+=' AND 0=1';else{where+=` AND p.id IN (${ids.map(()=>'?').join(',')})`;binds.push(...ids)}}
 const q=await a.d1.prepare(`SELECT p.id,p.name,p.status,p.start_date,p.end_date,pd.name product_name,COALESCE((SELECT GROUP_CONCAT(COALESCE(u.name,u.email), ', ') FROM project_managers pm LEFT JOIN users u ON u.id=pm.user_id WHERE pm.project_id=p.id), '-') pm_name,(SELECT COUNT(*) FROM tasks t WHERE t.project_id=p.id) task_count FROM projects p LEFT JOIN products pd ON p.product_id=pd.id WHERE ${where} ORDER BY CASE WHEN LOWER(COALESCE(p.status,'active')) NOT IN ${inactive} THEN 0 ELSE 1 END,pd.name,p.name`).bind(...binds).all(); const rows=(q.results??[]) as any[];
