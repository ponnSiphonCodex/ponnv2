...
export default async function ProjectsPage(){
 ...
 return <AppShell active="project" {...shellProps(a)}><PageHeader title="Project" subtitle={`Active ${rows.length} Project`} actions={a.scope.isPmo?<a className="btn-pink" href="/pm/projects/new" style={{textDecoration:"none"}}>เพิ่ม Project ใหม่</a>:undefined}/><div style={{padding:24}}><div className="project-list-summary"><div><b>Active Projects</b><span>{rows.length} รายการ</span></div>{a.scope.isPmo?<a className="btn-pink" href="/pm/projects/new" style={{textDecoration:"none"}}>สร้าง Project</a>:null}</div><div className="project-table-wrap"><table className="data-table"><thead><tr>{["ID","Product","Project","Project Manager","Timeline","Status","Tasks",""].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{rows.map(p=><tr key={p.id}><td>PRJ-{String(p.id).padStart(4,'0')}</td><td>{p.product_name??'-'}</td><td><b>{p.name}</b></td><td>{p.pm_name??'-'}</td><td>{fmt(p.start_date)} ถึง {fmt(p.end_date)}</td><td><span className="badge">{p.status??'Active'}</span></td><td>{p.task_count}</td><td><a href={`/pm/board?id=${p.id}`} style={{color:'#EC186E',fontWeight:600}}>เปิด Project</a></td></tr>)}{!rows.length&&<tr><td colSpan={8} style={{padding:32,textAlign:'center',color:'#6B7280'}}>ยังไม่มี Active Project</td></tr>}</tbody></table></div></div></AppShell>
}