...
export default async function ProjectsPage(){
 ...
 const q=await a.d1.prepare(`SELECT p.id,p.name,p.status,p.start_date,p.end_date,pd.name product_name,COALESCE((SELECT STRING_AGG(COALESCE(u.name,u.email), ', ') FROM project_managers pm LEFT JOIN users u ON u.id=pm.user_id WHERE pm.project_id=p.id), '-') pm_name,(SELECT COUNT(*) FROM tasks t WHERE t.project_id=p.id) task_count FROM projects p LEFT JOIN products pd ON p.product_id=pd.id WHERE ${where} ORDER BY pd.name,p.name`).bind(...binds).all(); const rows=(q.results??[]) as any[];
