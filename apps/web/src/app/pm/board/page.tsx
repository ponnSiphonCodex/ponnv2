...
export default async function BoardPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  return (
    <AppShell active="board" {...shellProps(a)}>
      ...
      {!board ? <div style={{ padding: 40, color: "#6B7280" }}>ไม่พบโปรเจกต์ที่เข้าถึงได้</div> : <BoardClient board={board} projects={projects} users={users} priorities={priorities} features={features} tags={tags} canWrite={aggregate ? false : canWrite} aggregate={aggregate} />}
    </AppShell>
  );
}
